...
export default function CategoryPage() {
  ...
  // Colors registry for the filter drawer
  const colors = [
...
function SidebarNavItem({ label, hasSub, isOpen, onClick }: { label: string, hasSub?: boolean, isOpen?: boolean, onClick?: () => void }) {
  return (
    <div 
      onClick={onClick}
      className="flex items-center justify-between text-[11px] font-bold tracking-[0.2em] text-gray-900 hover:opacity-60 transition-opacity cursor-pointer py-4"
    >
      <span>{label}</span>
      {hasSub && (
