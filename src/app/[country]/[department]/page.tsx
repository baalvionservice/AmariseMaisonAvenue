  ...
  return (
    <div className="bg-ivory pb-40">
      <section className="relative h-[70vh] w-full flex items-center justify-center overflow-hidden">
        <Image src={deptObj.imageUrl} alt={deptObj.name} fill className="object-cover opacity-80 animate-slow-zoom" priority />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          {deptCategories.map(cat => (
            <Link key={cat.id} href={`/${countryCode}/${department}/${cat.id}`} className="group relative aspect-[16/9] overflow-hidden bg-white shadow-luxury">
              <Image src={`https://picsum.photos/seed/amarise-cat-${cat.id}/1600/900`} alt={cat.name} fill className="object-cover transition-transform duration-[2s] group-hover:scale-105" />
