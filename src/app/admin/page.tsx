...
export default function AdminDashboard() {
  ...
  const hubIntelligence = useMemo(() => {
    return Object.values(regions).map(region => {
      ...
      else if (region.checkout > 15) aiInsight = "High conversion window — push private offers.";
  ...
  const globalHighIntent = useMemo(() => {
    ...
    <div className="space-y-10 animate-fade-in">
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-1">
          ...
          <h1 className="text-3xl font-headline font-bold text-slate-900 uppercase tracking-tight">Institutional Dashboard</h1>
          <p className="text-sm text-slate-500 max-w-2xl">Real-time global yield monitoring and curatorial oversight.</p>
        </div>
        
        <div className="flex items-center space-x-6">
          <div className="flex flex-col items-end text-right border-r border-slate-200 pr-6 mr-6">
             <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Aggregate Forecast (1h)</span>
             <span className="text-2xl font-headline font-bold text-emerald-600 italic tracking-tighter">+${(globalForecast / 1000).toFixed(1)}k</span>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        />
        <KPICard 
          label="Active Intent Users" 
          value={globalHighIntent.toString()} 
          trend="Critical" 
          trendUp={true} 
          icon={<Users className="text-teal-500" />} 
          subtext="High Purchase Risk"
        />
        <KPICard 
      ...
      <section className="relative h-[550px] w-full bg-[#0A1A2F] rounded-xl overflow-hidden shadow-2xl border border-white/5">
        ...
        <div className="absolute top-8 left-8 z-10 flex flex-col space-y-4">
          <div className="flex items-center space-x-3 bg-white/5 luxury-blur px-4 py-2 border border-white/10 rounded-md">
            ...
            <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-white">Global Network: ACTIVE</span>
          </div>
            ...
            >
              <div className="flex justify-between items-start mb-8">
                <div>
                  ...
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Node: {selectedRegion.id.toUpperCase()}</p>
                </div>
              ...
              <div className="space-y-6 flex-1 overflow-y-auto custom-scrollbar pr-2">
                ...
                <div className="space-y-4 pt-4 border-t border-white/5">
                   <p className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">Funnel Analytics</p>
      ...
      {/* Hub Intelligence Matrix */}
      <section className="space-y-8">
        <div className="flex items-center space-x-4">
           <div className="p-3 bg-plum/10 rounded-full text-plum"><BrainCircuit className="w-6 h-6" /></div>
           <div>
              <h2 className="text-2xl font-headline font-bold italic text-gray-900 uppercase tracking-tight">Hub Intelligence Matrix</h2>
        ...
        <div className="grid grid-cols-1 gap-8">
           {hubIntelligence.map(hub => (
             <HubIntelligenceCard key={hub.id} hub={hub} />
           ))}
        </div>
      </section>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 pb-20">
        {/* Action Matrix */}
        <Card className="lg:col-span-12 bg-white border-slate-200 shadow-sm luxury-card-lift">
...
function HubIntelligenceCard({ hub }: { hub: any }) {
  return (
    <Card className="bg-white border-slate-200 shadow-sm overflow-hidden group hover:border-plum transition-all duration-500">
       <div className="grid grid-cols-1 md:grid-cols-5 divide-x divide-slate-50">
          {/* Hub Identity Box */}
          <div className="p-6 bg-slate-50/30">
             <div className="space-y-1">
                <h4 className="text-xl font-headline font-bold italic text-gray-900">{hub.name}</h4>
                <p className="text-[8px] font-bold uppercase tracking-widest text-slate-400">Node ID: {hub.id.toUpperCase()}</p>
             </div>
          ...
          {/* Metric Box 1: Revenue Prediction */}
          <div className="p-6 space-y-4">
             <div className="flex items-center space-x-2 text-slate-400 group-hover:text-emerald-600 transition-colors">
                <Wallet className="w-3.5 h-3.5" />
                <span className="text-[9px] font-bold uppercase tracking-widest">Forecast (1h)</span>
             </div>
             <p className="text-2xl font-headline font-bold text-gray-900">${hub.predictedRev.toLocaleString()}</p>
             <p className="text-[8px] font-bold uppercase text-emerald-600">+12% Potential</p>
          </div>

          {/* Metric Box 2: High Intent */}
          <div className="p-6 space-y-4">
             <div className="flex items-center space-x-2 text-slate-400 group-hover:text-plum transition-colors">
                <Zap className="w-3.5 h-3.5" />
                <span className="text-[9px] font-bold uppercase tracking-widest">High Intent</span>
             </div>
             <p className="text-2xl font-headline font-bold text-gray-900">{hub.highIntent}</p>
             <p className="text-[8px] font-bold uppercase text-slate-400">Cart + Checkout</p>
          </div>

          {/* Metric Box 3: Archive Pressure */}
          <div className="p-6 space-y-4">
             <div className="flex items-center space-x-2 text-slate-400 group-hover:text-blue-600 transition-colors">
                <Heart className="w-3.5 h-3.5" />
                <span className="text-[9px] font-bold uppercase tracking-widest">Archive Pressure</span>
             </div>
             <p className="text-2xl font-headline font-bold text-gray-900">{hub.archivePressure}</p>
             <p className="text-[8px] font-bold uppercase text-slate-400">Wishlist Volume</p>
          </div>

          {/* Metric Box 4: AI Insight */}
          <div className="p-6 space-y-4 bg-plum/[0.02]">
             <div className="flex items-center space-x-2 text-plum">
                <BrainCircuit className="w-3.5 h-3.5" />
                <span className="text-[9px] font-bold uppercase tracking-widest">Curator AI</span>
             </div>
             <p className="text-[11px] text-gray-600 font-light italic leading-relaxed h-12 overflow-hidden">
               "{hub.aiInsight}"
             </p>
             <div className="flex justify-between items-center pt-2 border-t border-slate-100">
                <span className="text-[8px] font-bold uppercase text-slate-400">Conversion Index</span>
                <span className="text-[10px] font-bold text-plum">{hub.probability.toFixed(1)}%</span>
             </div>
          </div>
       </div>
       {/* Probability Bar */}
