...
export default function BrandIntegrityHub() {
  const { scopedBrandIntegrity, resolveBrandIntegrity, currentUser } = useAppStore();
  const { toast } = useToast();
  const [searchQuery, setSearchQuery] = useState('');

  const filteredIssues = scopedBrandIntegrity.filter(i => 
  ...
  const handleFix = (id: string) => {
    ...
    toast({ title: "Compliance Restored", description: "The regional narrative has been aligned with Maison standards." });
  };
    ...
    <div className="flex h-screen bg-ivory overflow-hidden font-body text-gray-900">
      <aside className="w-72 border-r border-border bg-white p-8 flex flex-col space-y-12 shadow-sm z-20">
        <div className="space-y-4">
          <div className="font-headline text-3xl font-bold tracking-tighter text-gray-900">
            AMARISÉ <span className="text-plum text-xs font-normal tracking-[0.4em] ml-2">BRAND</span>
          </div>
          <p className="text-[9px] text-gray-400 uppercase tracking-widest font-bold">Integrity Protocol</p>
        </div>
        
        <nav className="flex-1 space-y-1 overflow-y-auto pr-2 custom-scrollbar">
          <IntegrityNavItem icon={<LayoutDashboard />} label="Audit Registry" active={true} />
          <IntegrityNavItem icon={<Palette />} label="Visual Standards" active={false} />
          <IntegrityNavItem icon={<Type />} label="Voice & Tone" active={false} />
          <IntegrityNavItem icon={<BookOpen />} label="The 1924 Charter" active={false} />
        </nav>
      ...
      <main className="flex-1 overflow-y-auto bg-ivory relative">
        <header className="flex justify-between items-center bg-white/80 luxury-blur p-8 border-b border-border sticky top-0 z-30">
          <div>
            <h1 className="text-3xl font-headline font-bold italic text-gray-900 uppercase tracking-widest">Brand Integrity</h1>
            <p className="text-gray-400 text-[10px] tracking-widest uppercase font-bold mt-1">
              Institutional Heritage Audit • {currentUser?.country.toUpperCase()} Jurisdiction
            </p>
          </div>
          <div className="flex items-center space-x-6">
            <Badge variant="outline" className="bg-plum/5 text-plum text-[10px] uppercase tracking-widest border-plum/10 h-10 px-6 rounded-none">
              98.2% Consistency Score
            </Badge>
            <div className="w-10 h-10 bg-plum rounded-sm flex items-center justify-center font-headline text-xl font-bold italic text-white shadow-md">BI</div>
          </div>
        </header>
        ...
        <div className="p-12 space-y-12 animate-fade-in pb-32">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            <StatCard icon={<ShieldCheck />} label="Audited Items" value="1,240" trend="100% Core" positive />
            <StatCard icon={<AlertTriangle />} label="Active Deviations" value={filteredIssues.filter(i => i.status === 'pending').length.toString()} trend="Action Needed" positive={false} />
            <StatCard icon={<Zap />} label="AI Audit Velocity" value="850/s" trend="Optimal" positive />
            <StatCard icon={<Lock />} label="Compliance" value="Verified" trend="Maison Std" positive />
          </div>
          ...
          <Card className="bg-white border-border shadow-luxury overflow-hidden">
            <CardHeader className="border-b border-border flex flex-col md:flex-row md:items-center justify-between space-y-4 md:space-y-0 bg-ivory/10">
              <div className="space-y-1">
                <CardTitle className="font-headline text-2xl">Integrity Feed</CardTitle>
                <CardDescription className="text-[10px] uppercase tracking-widest font-bold text-gray-400">Regional deviations detected by AI Autopilot</CardDescription>
              </div>
              ...
              <Table>
                <TableHeader className="bg-ivory/50">
                  <TableRow>
                    ...
                    <TableHead className="text-[9px] uppercase font-bold">Issue Type</TableHead>
                    <TableHead className="text-[9px] uppercase font-bold">Deviation Description</TableHead>
                ...
                <TableBody>
                  {filteredIssues.map(issue => (
                    <TableRow key={issue.id} className={cn("hover:bg-ivory/30 transition-colors", issue.status === 'fixed' && "opacity-40")}>
                      <TableCell className="pl-8">
                        <div className="flex flex-col">
                          <span className="text-xs font-bold leading-tight uppercase tracking-tight">{issue.productName}</span>
                          <span className="text-[8px] text-gray-400 uppercase tracking-widest">{issue.country.toUpperCase()} Hub</span>
                        </div>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell className="text-center">
                        <Badge className={cn("text-[7px] uppercase tracking-tighter border-none", 
                          ...
                          issue.severity === 'medium' ? 'bg-orange-50 text-orange-600' : 
                      ...
                      <TableCell className="text-right pr-8">
                        {issue.status === 'pending' ? (
                          ...
                          <div className="flex justify-end items-center text-green-500 space-x-2">
                             ...
                             <span className="text-[8px] font-bold uppercase">Aligned</span>
                          </div>
                        )}
                      </TableCell>
                    </TableRow>
                    ...
                    <TableRow>
                      <TableCell colSpan={5} className="py-40 text-center">
                        <div className="flex flex-col items-center space-y-4 opacity-20">
                          ...
                          <p className="text-xs font-bold uppercase tracking-[0.2em]">All Regional Hubs Aligned with 1924 Charter</p>
                        </div>
                      </TableCell>
                    </TableRow>
                  )}
...
function IntegrityNavItem({ icon, label, active }: { icon: any, label: string, active: boolean }) {
  return (
    <button className={cn(
