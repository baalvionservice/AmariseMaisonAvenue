    ...
    <div className="flex h-screen bg-ivory overflow-hidden font-body text-gray-900">
      <aside className="w-72 border-r border-border bg-white p-8 flex flex-col space-y-12 shadow-sm z-20">
        ...
        <nav className="flex-1 space-y-1 overflow-y-auto pr-2 custom-scrollbar">
          ...
          <AutoNavItem icon={<Cpu />} label="Mitigation Matrix" active={false} />
          <AutoNavItem icon={<History />} label="System History" active={false} />
        </nav>
        ...
        <div className="p-12 space-y-12 animate-fade-in pb-32">
          <Tabs defaultValue="registry" className="w-full">
            <TabsList className="bg-white border border-border h-14 p-1 rounded-none mb-12">
              <TabsTrigger value="registry" className="rounded-none px-10 text-[10px] font-bold uppercase tracking-widest data-[state=active]:bg-plum data-[state=active]:text-white">Active Registry</TabsTrigger>
              <TabsTrigger value="mitigation" className="rounded-none px-10 text-[10px] font-bold uppercase tracking-widest data-[state=active]:bg-plum data-[state=active]:text-white">Mitigation Matrix</TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="mitigation" className="space-y-12 animate-fade-in">
                  ...
                  <div className="lg:col-span-8 space-y-8">
                     <div className="bg-plum/5 p-8 border border-plum/10 space-y-4">
                        <div className="flex items-center space-x-3 text-plum">
                           ...
                           <h3 className="text-[10px] font-bold uppercase tracking-[0.4em]">AI Strategic Mitigation</h3>
                        </div>
                        <p className="text-xs text-slate-600 italic leading-relaxed">
                          "Autonomous rules that trigger proactive business mitigation strategies when specific market thresholds are bypassed."
                        </p>
                     </div>
                  ...
                  <aside className="lg:col-span-4 space-y-8">
                     <Card className="bg-black text-white p-10 space-y-8 shadow-2xl relative overflow-hidden">
                        ...
                        <div className="relative z-10 space-y-6">
                           <h4 className="text-[10px] font-bold uppercase tracking-[0.4em] text-blue-400">Integrity Protocol</h4>
                           <p className="text-xs font-light italic leading-relaxed opacity-70">
                             "Every autonomous mitigation must be audited against the Founding Charter of 1924 to ensure brand purity is maintained."
                           </p>
                           <Button variant="outline" className="w-full border-blue-900/40 text-blue-400 rounded-none text-[9px] font-bold uppercase h-12">
...
function MitigationRuleCard({ hub, trigger, mitigation, impact }: { hub: string, trigger: string, mitigation: string, impact: string }) {
  return (
          ...
          <div className="space-y-4">
             <div className="flex items-center space-x-4">
                ...
                <div className="flex items-center space-x-2 text-red-500/60">
                   ...
                   <span className="text-[9px] font-bold uppercase tracking-widest">{trigger}</span>
                </div>
             </div>
             <div className="space-y-1">
                <h4 className="text-sm font-bold uppercase tracking-tight text-slate-900">{mitigation}</h4>
                <p className="text-[10px] text-slate-400 italic">Projected Impact: {impact}</p>
             </div>
          </div>
