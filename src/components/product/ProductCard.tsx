...
/**
 * ProductCard: Optimized gateway for high-value acquisitions.
 * Segregates the "Normal Archive" view from the "Private Order" narrative view.
 */
export const ProductCard = memo(({ product }: ProductCardProps) => {
  ...
  return (
    <article className="group relative flex flex-col bg-transparent overflow-hidden animate-fade-in h-full" aria-labelledby={`title-${product.id}`}>
      {/* NORMAL PATH: Archive View */}
      <Link href={`/${countryCode}/product/${product.id}`} className="block relative aspect-[3/4] overflow-hidden" aria-label={`View archive details for ${product.name}`}>
        ...
        <div className="absolute inset-x-0 bottom-0 p-8 flex flex-col space-y-3 translate-y-full group-hover:translate-y-0 transition-transform duration-1000 bg-white/95 backdrop-blur-xl z-20 border-t border-gray-100">
          {/* PRIVATE PATH: Cinematic Order Experience */}
          <Link href={`/${countryCode}/private-order/${product.id}`} className="w-full" onClick={(e) => e.stopPropagation()}>
            <Button 
              ...
              aria-label={`Request Private Acquisition for ${product.name}`}
            >
              <Lock className="w-3.5 h-3.5 mr-2" /> Private Acquisition
            </Button>
          </Link>
          <div className="flex space-x-2">
            ...
            >
              {isWishlisted ? "In Registry" : "Save Piece"}
            </Button>
            <Button 
              variant="outline" 
