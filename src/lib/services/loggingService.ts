...
class LoggingService {
  ...
  public log(params: Omit<SystemLog, 'id' | 'timestamp'>) {
    ...
    const color = entry.status === 'success' ? 'color: #10b981' : 'color: #ef4444';
    console.log(`%c[SYSTEM LOG] %c${entry.type.toUpperCase()} | ${entry.action}`, 'color: #7E3F98; font-weight: bold;', color, entry);
  ...
  public audit(params: Omit<AuditLogEntry, 'id' | 'timestamp'>) {
    ...
    console.log(`%c[AUDIT] ⚖️  ${entry.actorName} (${entry.actorRole}) performed ${entry.action} on ${entry.entity}`, 'color: #D4AF37; font-weight: bold;', entry);
    return entry.id;
  }
  ...
  public ai(params: Omit<AIActionLog, 'id' | 'timestamp'>) {
    ...
    console.log(`%c[AI TRACE] 🧠 Module: ${entry.moduleId} | Confidence: ${entry.confidence || 'N/A'}`, 'color: #3B82F6; font-weight: bold;', entry);
    return entry.id;
  }
  ...
  public error(params: Omit<MaisonError, 'id' | 'timestamp' | 'resolved'>) {
    ...
    console.error(`%c[ANOMALY] 🚨 ${entry.type} in ${entry.module}: ${entry.message}`, 'color: #ef4444; font-weight: bold;', entry);
    return entry.id;
  }
