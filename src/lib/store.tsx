...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const [integrations, setIntegrations] = useState<MaisonIntegration[]>(INTEGRATIONS.map(i => ({ ...i, brandId: activeBrandId })));
  ...
  const upsertProduct = (p: Product, changeSummary: string = 'Institutional registry update') => {
    setProducts(prev => {
      ...
      if (idx > -1) {
        ...
        const newVersion: ArtifactVersion = { id: `v-${Date.now()}`, version: existing.currentVersion + 1, data: JSON.parse(JSON.stringify(existing)), editedBy: currentUser?.name || 'System', timestamp: new Date().toISOString(), changeSummary };
        const updated: Product = { ...p, currentVersion: existing.currentVersion + 1, versionHistory: [newVersion, ...existing.versionHistory].slice(0, 50), lastSyncedAt: new Date().toISOString(), lastEditedRegion: (activeHub as any) || 'global' };
  ...
  const executeSafeSync = (categories: SyncCategory[], targets: CountryCode[]) => {
    const snapshot: GlobalSyncSession['snapshotBefore'] = { products: JSON.parse(JSON.stringify(products)), seo: JSON.parse(JSON.stringify(seoRegistry)), config: JSON.parse(JSON.stringify(countryConfigs)) };
    const session: GlobalSyncSession = { id: `sync-${Date.now()}`, timestamp: new Date().toISOString(), categories, targets, snapshotBefore: snapshot, actorName: currentUser?.name || 'Super Admin', status: 'applied' };
    if (categories.includes('products')) setProducts(prev => prev.map(p => p.scope === 'global' ? { ...p, regions: targets, lastSyncedAt: session.timestamp } : p));
    setGlobalSyncHistory(prev => [session, ...prev]);
    logAction(`Executed Safe Global Sync`, `Targets: ${targets.join(', ')}`, 'global', 'medium');
  };
  ...
  const value = useMemo(() => ({
    ...
    rollbackProductVersion: (productId: string, versionId: string) => setProducts(prev => prev.map(p => { if (p.id === productId) { const version = p.versionHistory.find(v => v.id === versionId); if (version) return { ...version.data, versionHistory: p.versionHistory, currentVersion: p.currentVersion, lastSyncedAt: new Date().toISOString() }; } return p; })),
    lockProductForEditing: (productId: string) => { setProducts(prev => prev.map(p => p.id === productId ? { ...p, editingLock: { userId: currentUser?.id || 'sys', userName: currentUser?.name || 'Sys', expiresAt: new Date(Date.now() + 15*60000).toISOString() } } : p)); return true; },
    unlockProduct: (productId: string) => setProducts(prev => prev.map(p => p.id === productId ? { ...p, editingLock: undefined } : p)),
    ...
    runWorkflowTask: (id: string) => { setWorkflows(prev => prev.map(w => w.id === id ? { ...w, status: 'running' } : w)); setTimeout(() => setWorkflows(prev => prev.map(w => w.id === id ? { ...w, status: 'complete', lastRun: new Date().toISOString() } : w)), 1000); },
    runWorkflowSequence: (name: string, country = 'global') => workflows.filter(w => w.country === country || w.country === 'global').forEach(w => value.runWorkflowTask(w.id)),
    submitApproval: (type: ApprovalRequest['contentType'], id: string, country = 'global') => setApprovalRequests(prev => [{ id: `a-${Date.now()}`, userId: currentUser?.id || 'sys', userName: currentUser?.name || 'Sys', contentType: type, contentId: id, country, status: 'pending', timestamp: new Date().toISOString() }, ...prev]),
    handleApprovalAction: (id: string, a: boolean) => setApprovalRequests(prev => prev.map(r => r.id === id ? { ...r, status: a ? 'approved' : 'rejected' } : r)),
    ...
    runQATest: (id: string) => { setQaTests(prev => prev.map(t => t.id === id ? { ...t, status: 'running' } : t)); setTimeout(() => setQaTests(prev => prev.map(t => t.id === id ? { ...t, status: 'passed', lastRun: new Date().toISOString() } : t)), 1500); },
    runAllQATests: () => qaTests.forEach(t => value.runQATest(t.id)),
    runStressTest: (load: number, type: StressTest['type']) => { const id = `stress-${Date.now()}`; const test: StressTest = { id, name: `Stress: ${type}`, loadSize: load, type, status: 'running', metrics: { startTime: new Date().toISOString(), processedCount: 0, errorCount: 0, failureCount: 0 }, logs: [], country: activeHub }; setStressTests(prev => [test, ...prev]); setTimeout(() => setStressTests(prev => prev.map(t => t.id === id ? { ...t, status: 'passed', metrics: { ...t.metrics, processedCount: load, durationMs: 1200 } } : t)), 2000); },
